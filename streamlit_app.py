...
def preprocess_input(data):
    ...
    # Convert Ethnicity to numerical values using a dictionary mapping
    ethnicity_mapping = {
        'Ethnic Group 1': 0,
        'Ethnic Group 2': 1,
        'Ethnic Group 3': 2,
    ...
    data['Ethnicity'] = ethnicity_mapping.get(data['Ethnicity'], -1)  # Default to -1 if not found
...
def main():
    ...
    if submit_button:
        ...
        # Display the result
        if prediction[0] == 1:
            st.write(f"RESULT: This patient has Alzheimer Disease")
        else:
            st.write(f"RESULT: This patient does not have Alzheimer Disease")
